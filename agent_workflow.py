...
jailbreak_fail_agent = Agent(
  ...
  instructions="make sure that the user's input is within the context of Milleu Insight's support FAQ and policies. Do not by all means give them the answer to their question",
  model="gpt-4.1",
  model_settings=ModelSettings(
    temperature=1,
    top_p=1,
...
async def run_workflow(workflow_input: WorkflowInput):
  with trace("Milieu Agent"):
    ...
    if guardrails_hastripwire:
      ...
      jailbreak_fail_agent_result = {
        "output_text": jailbreak_fail_agent_result_temp.final_output_as(str)
      }
    else:
