...
def get_retention_offers(
    ...
    # TODO: Implement real offer logic / lookup
    # Stub response for now
    return {"offer": {"discount_percent": 20, "duration_months": 12}}


# =============================================================================
# Shared client for guardrails and runtime context
# =============================================================================
client = AsyncOpenAI()
        ...
        {"name": "NSFW Text", "config": {"model": "gpt-4.1-mini", "confidence_threshold": 0.7}},
        {
            ...
            "config": {
                "system_prompt_details": (
                    "You are a customer support assistant. Raise the guardrail if questions aren’t focused on "
                    "customer inquiries, product support, and service-related questions."
                ),
                "model": "gpt-4.1-mini",
...
def get_guardrail_safe_text(results: list[Any] | None, fallback_text: str) -> str:
    ...
    pii = next(
        (
            ((r.info if hasattr(r, "info") else {}) or {})
            for r in (results or [])
...
async def scrub_conversation_history(history: list[dict[str, Any]] | None, config: dict[str, Any] | None) -> None:
    try:
...
async def scrub_workflow_input(workflow: dict[str, Any] | None, input_key: str, config: dict[str, Any] | None) -> None:
    try:
        guardrails = (config or {}).get("guardrails") or []
        pii = next((g for g in guardrails if (g or {}).get("name") == "Contains PII"), None)
        if not pii:
    ...
    mask_pii = (
        next(
            (
                g
                for g in guardrails
                if (g or {}).get("name") == "Contains PII" and ((g or {}).get("config") or {}).get("block") is False
            ),
            None,
        )
        is not None
...
def build_guardrail_fail_output(results: list[Any]) -> dict[str, Any]:
    def _get(name: str):
        for r in (results or []):
    ...
    def _tripwire(r) -> bool:
        return bool(getattr(r, "tripwire_triggered", False))

    def _info(r) -> dict[str, Any]:
        return getattr(r, "info", None) or {}
...
classification_agent = Agent(
    ...
    instructions=(
        'Classify the user’s intent into one of the following categories: "return_item" or "get_information".\n\n'
...
async def run_workflow(workflow_input: WorkflowInput) -> dict[str, Any]:
    with trace("Milieu Agent"):
        ...
        if guardrails_hastripwire:
            jailbreak_fail_agent_result_temp = await Runner.run(
                jailbreak_fail_agent,
        ...
        classification = classification_agent_result["output_parsed"].get("classification")
        ...
        if classification == "get_information":
            ...
            return {
                "guardrails": guardrails_output,
                "classification": classification_agent_result,
                "agent": information_agent_result,
                "state": state,
            }

        # Fallback if classification is unexpected
        return {
            "guardrails": guardrails_output,
            "classification": classification_agent_result,
            "state": state,
